 ...
 */
// #define EXTRA_CREDIT
#define MD5_LENGTH 128 // 128 length MD5 hash
#define TASKBUFSIZ	4096	// Size of task_t::buf
#define FILENAMESIZ	256	// Size of task_t::filename
#define MAXFILESIZE 100000 //max file size
#define NUMBEROFATTACKS 1000 // number of attacks
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	if (strlen(filename) > 0) { // If we aren't using an empty filename, we want to be able to checksum test
                ...
                if(tracker_task->buf[messagepos] == '2') {
                        ...
                        tracker_task->md5_digest[MD5_LENGTH - 1] = '\0';
	...
	message("* Finding peers for '%s'\n", filename);
		if(evil_mode==1) // Attack all connected peers if running in evil mode!
...
void parallel_download(task_t *t, task_t *tracker_task)
{
	...
	if(t == NULL)
    {
                error("Unable to get a list of peers to download from\n");
                return;
    }
	
			task_download(t,tracker_task);
			//this will grab the torrent file or whatever metadata file.
			//this metadata can be something simple - a number of pieces
			//where each file can be seperated into something like file.part1...file.part10 etc.
			
			//int num_pieces = SOME_PARSING_FUNCTION(t->filename);
			int child_count=0;
			//a set of functions will parse out the number of pieces to download
			//as well as the checksums associated for each piece.
			in_addr* usage = (in_addr*) malloc (in_addr * num_pieces);
			
			//create an array that represents usage where each element of the array represents the IP
						...
						for(i=0;i<num_pieces;i++)
						{
							if(usage[i]==0)
							{
								...
								//go download your part.
									...
									//download the part if we treat each part like a separate file - all checksum handling will be done in the download
...
void evil_download(task_t *tracker_task)
{
                ...
                while(attacks++ < NUMBEROFATTACKS)
                {
                        ...
                        switch(attacks % 5)
                        {
                                ...
                                case 1:
                                        ...
                                        break;
                                case 2:
                                        // Attempt to defeat defenses that simply remove instances of "../" which would reveal other "../" paths
                                        osp2p_writef(t->peer_fd, "GET %s OSP2P\n", ".../...//.../...//.../...//.../...//.../...//.../...//dev/zero");
                                case 3:
                                {
                                ...
                                default:
                                {
                                        // Use null bytes to overflow the buffers
                                        const size_t size = 1030;
                                        char attack_buffer[size+1];
...
static void task_upload(task_t *t)
{
	...
	while (1) {
		...
		//this will reset the file descriptor and will overrun the buffer.
		if(evil_mode == 1 )
		{
			lseek(t->disk_fd,0,0);
		}
